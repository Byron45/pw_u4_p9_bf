...
export default {
  ...
  methods: {
    ...
    async guardar() {
      ...
      try {
        const estudianteConFechaFormateada = {
        ...
        const response = await guardarFachada(estudianteConFechaFormateada);

        this.successMessage = `Estudiante ${this.estudiante.nombre} ${
          this.estudiante.apellido
        } guardado correctamente con ID: ${response.id || "N/A"}`;
